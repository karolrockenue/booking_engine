...
**Last updated:** 2026-06-10 **Status:** Phases 1–5 \+ 6.5 \+ 6.6 \+ 7.1 shipped. **Robust fulfilment 0a + 0b deployed to `main` (create-before-pay; 0b awaiting UI click-throughs) — see the 2026-06-09 session logs + the async-checkout handoff.** **Second storefront template — `street-ivory` (Street · Ivory) — ✅ END-TO-END 2026-06-04 (home + rooms + extras + checkout + confirmation all in Street; booking logic reused, only the skin differs — see the 2026-06-04 + 2026-05-27 session logs). Admin Media / Content / Design tabs are now template-aware via per-template content schemas.** **Third storefront template — `editorial-calm` (Editorial · Calm) — ✅ END-TO-END 2026-06-10, browser-verified on the Mews demo (renamed "Mason & Fifth" for evaluation); ships its own seed content + bundled mockup photography so a fresh hotel looks exactly like the design mockup. Uncommitted, awaiting Karol's visual pass — see §25 + the 2026-06-10 session log.** **Payment rail = Ryft (Stripe fully removed) — cutover merged to `main` 2026-06-29, deploying to Railway on sandbox keys; see the Ryft companion-doc callout below.** Flex auto-charge \+ PMS retry recovery live on production (now Ryft). **Guest comms (Phase 7.1) shipped end-to-end — Unlayer composer + R2 image pipeline + scheduler + send log. Maily.to replaced 2026-05-12 (no font control); Unlayer integrates uploads into the Media library and supports brand fonts. See §13.** Welcome Pickups partnership in motion. **✅ Cloudbeds Marketplace CERTIFIED 2026-05-20** — passed via the live Marketplace "Connect app" flow. The retry-day fixes that closed it: the OAuth callback now accepts **Cloudbeds-initiated** installs (unsigned state → fresh property, `63d5745`); the admin "Open site" button uses the `/<slug>` path, not the retired `?property=` shim (`8c4079b`); and the Cloudbeds console's **Login URL** (a stale `market-pulse.io` value) was repointed to `/api/install`. Full chain verified end-to-end: OAuth → auto-create hotel → inventory/rates/extras/hotel-details sync → webhooks → Stripe auto-attach → real NR + Flex bookings with per-morning breakfast folio + cancel. **us2 needs no special handling** — OAuth authorize is the standard `hotels.cloudbeds.com/api/v1.3/oauth` (not `us2`/`v1.1`); the cluster resolves internally. See the **2026-05-20 session log** immediately below + §7 chronicle. **Post-cert cleanup DONE 2026-06-04: dropped the `cert_attach_test_stripe` trigger + function and deleted the dead duplicate test property — kept one test hotel (`rockenue-partner-account-15cece68`, live token) + Lancaster Court. Only the "already connected?" install guard remains.** **2026-05-21 (post-cert):** platform domain **`app.rockenue.tech`** is live (Hostinger DNS → Railway) with a Rockenue Tech landing page; admin gained **room-type visibility** + a dedicated **Rooms** page, and **Rate plans** now show logical (deduped) plans with per-plan visibility + a booking-engine display-name override. **These rate-plan controls (which-to-display via `is_public` + booking-engine `display_name`) are PMS-agnostic — verified to work for Mews properties too (sync never overwrites the admin's choices; storefront serves `display_name ?? name_public ?? name`); admin Rates copy made PMS-aware 2026-06-09 (`c7b1754`). See the Mews handoff "Rates (read)" note.** **Google Hotel Center integration Sprints 1–4 + Sprint 5 foundation shipped** (separate work stream — see `Google Hotel Center — Blueprint.md`). See the **2026-05-21 session log** below. **Mews PMS integration started 2026-06-05 (separate work stream — see `Mews Integration — Plan for Review.md`): Phase 1 (provider-neutral `PmsAdapter`, Cloudbeds refactored behind it, zero behaviour change) + Mews client foundation + Phase 2 (per-property `pms_type`/`pms_credentials`, Mews-native inventory tables, admin Mews connect with encrypted token) + Phase 3 (Mews read path: `MewsAdapter` reads rooms/rates/availability/pricing into the `mews_*` tables, `getAvailability` joins category × rate at read time, and `computeAvailability` dispatches to the adapter on `pms_type`) + Phase 4 (Mews write path — create reservation with per-night TimeUnitPrices, record external payment referencing the Stripe PI, cancel; all verified through the adapter on the demo) shipped. Built/tested against the public Mews demo — no Mews contact needed until certification. **Production is gated on CQ-1/CQ-2; folio extras + staff notes are a known follow-up.**
...
> **Companion blueprint — Google Hotel Center.** The Google integration work stream (Free Booking Links / Hotel Ads — Hotel List XML feed, ARI Push, Landing Pages XML, JSON-LD hotel-price data, Connectivity Partner application) lives in its **own** document: **`Google Hotel Center — Blueprint.md`** in the repo root. If a task touches Google feeds, Hotel Center, FBL/Hotel Ads, or hotel-price structured data, read that file — it's the source of truth for that work stream. This document stays focused on the IBE (booking engine, Cloudbeds, Ryft payments, admin, themes, email).
...
> **Companion doc — Ryft payment rail (replaced Stripe).** The booking engine's payment rail is now **Ryft** (Stripe wouldn't onboard the UAE platform to pay UK hotels). The work stream lives in **`Ryft Migration — Handoff.md`** (repo root). **Status (2026-06-29/30): CUTOVER COMPLETE — Stripe fully deleted, Ryft is the sole rail (NR + Flex, refunds/void/cancel, off-session auto-charge, re-enter-card recovery, currency-display fix). Merged to `main` (`80c50f5`); Railway (app.rockenue.tech) deploys it on sandbox keys.** Sandbox-proven end-to-end on Cloudbeds and Mews. **`paymentRail` was removed** — checkout is unconditionally Ryft (only Ryft-active properties can take payment). The many "Stripe" references in the dated session logs below are **historical** — they describe `main` before the cutover; for anything payments-related the handoff is the source of truth. Remaining work is **go-live only** (live Ryft account → live keys, prod webhook + `RYFT_WEBHOOK_SECRET`, per-hotel onboarding) — handoff §7/§9.

> **Companion doc — Robust fulfilment / async checkout.** The booking→PMS fulfilment rework lives in **`Robust Fulfilment (Async Checkout) — Handoff.md`** (repo root). **Steps 0a + 0b are DONE** (0a idempotent `fulfilBooking`; 0b create-before-pay — row + extras intent persisted at `init`, details patched before the charge, finalise verifies + fulfils, webhook hardened to backfill Flex payment method + flip status on a browser-death rescue). 0b is **backend-verified + deployed to `main`, awaiting Karol's 3-theme UI click-throughs** (handoff §10). **Phase 2 (202 + status poll) NOT started** — mockup ready for review (`Phase2-Confirmation-Finalising-EditorialCalm.html`), needs sign-off on the sold-out-after-confirm trade-off. Read the handoff before touching `/api/bookings`, the Stripe webhook, the checkout clients, or `fulfilBooking`. See the 2026-06-09 "Robust fulfilment Step 0b" + "Step 0a" session logs below.

---

## **2026-06-10 session log — third template (Editorial Calm) end-to-end**

The Mason & Fifth design exploration (iterated as click-through HTML mockups in `export-src/`, served via `python3 -m http.server 8080`) became the third storefront template. Design decisions were locked mockup-first over many rounds (see §25), then ported in one pass.

### Editorial Calm (`editorial-calm`) — boutique-guesthouse voice
- Off-white paper (`#FAF9F5`), near-black ink, warm hairlines, pill buttons. Three faces with fixed roles via next/font: **Hanken Grotesk** (structure/titles/prices), **Newsreader** (oversized editorial serif), **Courier Prime** (bracketed mono labels `( LIKE THIS )`, body captions).
- **Files:** `src/themes/editorial-calm/` mirrors Street — `tokens.ts`, `fonts.ts`, `schema.ts`, `content-defaults.ts`, `stripe-appearance.ts`, `index.ts` (barrel + `activeEditorialCalmTokens()`), `EditorialCalmShell.tsx`, `components/` (`primitives`, `Nav` + property-name `Wordmark`, `BookingChrome` = StepBar/StepDots/PageHeading, `DatePicker`, `SearchBar`, `Newsletter`), `screens/` (`Home`, `RoomSelect`, `Extras`, `Checkout`, `Confirmation`).
- **Signature screen — Extras ("How you'll stay"):** photo collage (1 large + 2 stacked) of the held room → room name + description → dominant rate-plan rows (guest can switch plan here; availability re-fetched, rate-plan rows show `supportingNoteFor()` copy) → dotted-leader extras (menu typography, a clear register below the rates) → basket rail right + **floating corner basket** (fixed bottom-right chip with live total, expands to a mini basket card).
- **Home hero = the C1 "Editorial" mockup, faithfully:** full-bleed photo + scrim, bracket eyebrow, serif headline, LOCATION · DATES · GUESTS white underline fields with paper popovers (calendar/steppers), `( REDEEM A VOUCHER )` secondary link (visual; code carried as `?voucher=` — redemption is future Cloudbeds-vouchers work), and the **STAYING LONGER?** pill → long-stay panel (arrival month + length chips; CTAs are a pre-filled enquiry mailto until a residency flow exists).
- **Booking logic reused verbatim** from `@/lib/booking` + `StripePaymentSection` (incl. 0b create-before-pay `initBooking` → `patchBookingDetails` → confirm). Only the skin is new.

### Per-template content defaults (new capability)
- `getPropertyContent(propertyId, defaults?)` / `mergeContent(blocks, base?)` now accept a base — **Editorial Calm ships its own seed copy** (`ecDefaultContent`: "A little less ordinary.", the M&F manifesto, neighbourhood/practicalities voice) instead of inheriting Portico's defaults. DB content blocks still override field-by-field. Manifesto renders `hero.pressQuote` + attribution (fields Portico used for the press quote).
- **Bundled photography defaults:** the mockup's warm 35mm set copied to `public/editorial-calm/` (hero `paper.jpg`, gallery, per-room fallbacks for carousel + collage) — a fresh property looks like the mockup until R2 uploads override.

### Registration + verification
- Registered in `active-theme.ts` (`EDITORIAL_CALM_THEMES` + `isEditorialCalm`), `template-schema.ts`, admin Design tab `TEMPLATES`; all six storefront routes branch on it (`/book` redirects home — booking form lives in the hero).
- Verified with Playwright end-to-end on `mews-demo-hotel` (renamed **"Mason & Fifth"** in the dev DB for evaluation — revert when done): home → dates → rooms → extras (rate switch, extra toggle, floating basket) → checkout (Stripe element, create-before-pay row created, button gating) — no console errors. Fixes from verification: "My rate rate" label dedup, border-shorthand React warning, LOCATION popover anchored under its field.
- **Deferred:** room tagline subheading (no data field), Phase-2 finalising/poll confirmation states (robust-fulfilment stream), newsletter has no list backend (local confirm only), `/dev/themes` stale, mockup's multi-house "Three places to call home" section (needs a houses content block — fights the one-hotel silo; ask Karol).

---

## **2026-06-09 session log — Robust fulfilment Step 0b (create-before-pay) — DEPLOYED, awaiting UI test**

Closes the "browser died before `/api/bookings`" window: the booking row + extras intent now exist server-side **before** the card is charged, so fulfilment never depends on the browser surviving. Backend-verified, committed, and **deployed to `main`** (Railway); the 3-theme UI click-throughs are Karol's remaining step (handoff §10). Synchronous checkout UX unchanged (still returns the reservation #).

### Shipped (backend, verified by smokes)
- **Three server writes around the card.** `POST /api/bookings/init` (on email entry) → `prepareBooking` + **`status:"pending"`** row (name placeholders; email optional — some themes render Stripe before email) + day-rates + extras intent + Stripe intent; idempotent on `orderId`. `POST /api/bookings/[id]/details` patches guest name/country onto the pending row **before** the charge (throws so we never charge a card we couldn't attach details to). `POST /api/bookings` refactored to **verify-and-fulfil** — find by `orderId`, verify the Stripe intent, flip `pending → paid/payment_authorized`, `fulfilBooking`; **create-if-absent fallback** kept for pre-0b/standalone callers.
- **No-drift extractions.** `src/lib/booking/prepare-booking.ts` (validation + availability re-check + price split + cancellation snapshot + `chargeAt` + extras-intent builder, used by **both** init and finalise); `src/lib/stripe/intents.ts` (the two standalone intent routes are now thin wrappers); `src/lib/stripe/rescue-booking.ts` (extracted, unit-testable webhook rescue).
- **Webhook hardening (closed a real durability gap).** On a browser-death rescue the webhook now **backfills** missing Stripe state — critically the **Flex saved payment method** (`si.payment_method`), without which the auto-charge cron skips the booking forever — and **flips** `pending → paid/payment_authorized` so the retry cron can pick it up even if the immediate (age-gated) rescue defers.
- **Client lib + 3 themes.** `initBooking()` + `patchBookingDetails()` added; default / Portico / Street checkouts wired to init → patch → confirm card → finalise. Stripe mount unchanged.
- **Abandoned-cart sweep (follow-up).** `POST /api/cron/cleanup-pending` + `src/lib/booking/sweep-abandoned.ts` mark stale `pending` init rows `"abandoned"` (non-destructive; only pending + unreserved + >24h + no succeeded payment). Ops step: register the hourly schedule.
- **No schema change** (reused 0a's columns).

### Verified
`tsc` + `eslint` clean (only the pre-existing `property.slug` warnings). Smokes: `bookings-init-smoke` PASS (prepareBooking correctness; NR + Flex browser-death rescue through the **real** `rescueStuckBooking`, incl. the Flex PM backfill + age-gate; idempotency), `sweep-abandoned-smoke` PASS, `mews-fulfil-smoke` PASS, `mews-retry-smoke` 13/13. Commits `cb78285` + `e57b8c0` on `main`.

### Next
- **Karol:** the 3-theme × NR+Flex × Mews+Cloudbeds UI click-throughs (handoff §10), incl. the kill-the-tab-after-charge durability check. Register the `cleanup-pending` cron schedule.
- **Phase 2** — `/api/bookings` returns **202**; confirmation page polls `/status` ("finalising…"). Mockup ready (`Phase2-Confirmation-Finalising-EditorialCalm.html`); needs sign-off on the sold-out-after-confirm trade-off before build.
...
### Shipped
- **Phase 3 — reads (`58d2aa2`).** `MewsAdapter` read path + factory dispatch on `pms_type`. New `src/lib/pms/mews/`: `credentials.ts` (decrypt the stored AccessToken), `sync-inventory.ts`, `availability.ts`, `sync-hotel-details.ts`. `resourceCategories/getAll` → `room_types`; `rates/getAll` (active+enabled+public of the service) → `rate_plans`; legacy `services/getAvailability` → `mews_category_availability`; `rates/getPricing` → `mews_rate_prices`; `getAvailability` joins category × rate at read time. `computeAvailability` now dispatches to the adapter on `pms_type`, so storefront + Google JSON-LD + ARI are all PMS-agnostic.
- **Phase 4 — writes (`448a41c`).** `mews/reservations.ts`: `createReservation` (`customers/add` + email dedupe → `reservations/add`, per-night `TimeUnitPrices`, `CheckRateApplicability:false`, `CheckOverbooking:true`), `recordPayment` (`payments/addExternal`, `ExternalIdentifier`=Stripe PI, Gross/Net by tax mode), `cancelReservation` (dedicated `reservations/cancel`, fee off). Interface gained `nightlyRates` + `externalIdentifier`.
- **Booking-flow de-coupling (`9868baf`, `ddb5148`).** The first real Mews booking flushed out **5 Cloudbeds-coupled assumptions** in the *shared* flow — all fixed: bookings/route availability re-check (queried Cloudbeds `inventory` + required `cloudbedsPropertyId` → now PMS-aware via adapter), `chargeBooking` + `autoCancelAfterGrace` + cancel-route connection guards (required `cloudbedsPropertyId` → now allow `pms_type='mews'`), and a Stripe `sanitizeEmail` fix (a half-typed email reached Stripe → "Invalid email" + poisoned the per-orderId idempotency key).

### Decisions locked this session
- **Rate↔category model = Option A.** A Mews Rate is **service-wide, not category-bound** (confirmed via live `rates/getAll`, Mews docs, and Market Pulse's own adapter). One `rate_plans` row per Rate with **`room_type_id = NULL`**; join category × rate at read via `mews_rate_prices`.
- **Availability = legacy `services/getAvailability`.** The versioned `/2024-01-22` rejects our call (`Invalid Metrics`); the legacy `Availabilities[]` is the absolute netted sellable count (Market Pulse's proven path).
- **Build P4 on demo in parallel** by explicit decision, accepting that a CQ-1 "no" could force write-path rework.

### Demo full lifecycle VERIFIED (2026-06-06/07)
On `mews-demo-hotel` (GB test Stripe Connect account + synced inventory): storefront search → **Flex booking** → Mews reservation **#75697 Confirmed** → **auto-charge** → card charged → **external payment recorded in Mews** (ExternalPayment, Charged, £125) → **guest cancel** → Mews **State=Canceled** + **Stripe refund £125** (reverse_transfer) → booking `cancelled`. Verified from our side via the Connector API at each step. **Lesson:** any shared booking/charge/cancel path touched for Mews likely hides `cloudbedsPropertyId`/`inventory`-table assumptions — grep for them.

### Remaining (Mews)
- **P5** — webhooks (`ServiceOrderUpdated` + `ResourceBlockUpdated`) + availability polling. **P6** — certification + dedicated prod integration, **gated on CQ-1/CQ-2**.
- **Deferred follow-ups (non-blocking):** folio extras + staff-note posting (Mews extras live on a separate product service — `orders/add` rejects the accommodation ServiceId); compensating external-payment reversal in Mews on cancel (§7.4 — Stripe refunds, but the recorded payment stays on the folio); capture the sold-out (`CheckOverbooking`) error shape for UX.
- **Unrelated/pre-existing:** admin Theme tab crashes on an empty `theme` jsonb (all properties; theming is template-based now) + no "Save Changes" success toast.

---

## **2026-06-05 session log — Mews PMS integration (Phases 1–2)**

Adding **Mews** as a second PMS, fully siloed from Cloudbeds. Full design + Mews-reviewed plan: **`Mews Integration — Plan for Review.md`** (carries the standing plan + the §13 review corrections + §0a certification questions). Also this session: Street finished **end-to-end** (see the 2026-06-04 entry / §3 below) and the **post-cert cleanup** was completed (cert Stripe trigger dropped + duplicate test property deleted).

### Decisions
- **Connector API** (not Distributor): keep **Stripe Connect** as the money rail and **record an external payment** in Mews (`payments/addExternal`) — mirrors the Cloudbeds model. Pending Mews's blessing at certification (CQ-1).
- **Clean adapter:** one `PmsAdapter` interface; Cloudbeds + Mews are two impls behind `getPmsAdapter(property)` keyed on `properties.pms_type`.
- **Per-PMS native storage:** Mews availability/price live in their own `mews_*` tables, not forced into the Cloudbeds `inventory` shape; the adapter joins them at read time.
- **Dev on the public Mews demo** (`api.mews-demo.com`) — no Mews contact needed until certification.

### Shipped
- **Phase 1 (`62b203d`):** `src/lib/pms/` — `PmsAdapter` interface + `CloudbedsAdapter` (thin delegation, zero behaviour change) + factory. Write path (bookings create/cancel, auto-charge, sync-pending-extras, OAuth-callback sync) routed through the adapter. Verified by a live Cloudbeds booking (Lancaster → reservation `8600171440767`, `pms_synced`).
- **Mews client foundation (`77ac4c2`):** `lib/pms/mews/client.ts` (auth triple, cursor pagination, 429/403 backoff), `timezone.ts` (pure-Intl `toMewsUtc`/`utcToLocalDate`, BST-safe), `scripts/mews-probe.ts`. Verified against the gross demo enterprise.
- **Phase 2 (`5ef30b4`):** migration — `pms_type`, `pms_credentials` (jsonb, encrypted token), `mews_category_availability`, `mews_rate_prices`, neutral `pms_reservation_id`/`pms_payment_id`/`pms_item_id`. Admin **Mews** connect tab: paste AccessToken → validate → pick Reservable service + external payment type → connect. Verified end-to-end via the UI against a dedicated `mews-demo-hotel` (stored `pms_type=mews`, encrypted token, Gross, OnlinePayment).
- **Phase 3 (`58d2aa2`) — Mews reads:** `MewsAdapter` (factory returns it for `pms_type='mews'`; writes/webhooks throw until P4/P5) + `mews/credentials.ts`, `sync-inventory.ts`, `availability.ts`, `sync-hotel-details.ts`. **Rate model = Option A (locked):** a Mews Rate is service-wide, so `rates/getAll` (active+enabled+public of the service) → `rate_plans` with **`room_type_id = NULL`**; `resourceCategories/getAll` → `room_types`; legacy `services/getAvailability` `Availabilities[]` → `mews_category_availability`; `rates/getPricing` `CategoryPrices[].AmountPrices[]` (Gross/Net by tax mode) → `mews_rate_prices`. `getAvailability` joins category × rate at read time. **`computeAvailability` now dispatches to the Mews adapter on `pms_type`**, so the storefront + Google JSON-LD + ARI are all PMS-agnostic. Versioned `getAvailability/2024-01-22` rejected (`Invalid Metrics`) → use legacy (plan §8). Confirmed the property-wide-rate model three ways (live API, Mews docs, Market Pulse `mews.sentinel.adapter.js`). Verified e2e on the gross demo via `scripts/mews-sync-smoke.ts`: 121 rooms, 15 rate plans, ~15.5k native rows (7-day window), 1,065 joined room×rate options with correct nightly prices; throwaway property cleaned up, Cloudbeds untouched. `syncExtras` deferred to P4 (`property_extras` is Cloudbeds-shaped).
...
## **25\. Editorial Calm template (plan 2026-06-10)**

Third storefront theme, ported from the `export-src/` HTML mockups (Mason & Fifth design exploration). Status: **shipped 2026-06-10** — full mockup parity incl. hero booking form (LOCATION/DATES/GUESTS + voucher link + STAYING LONGER panel), per-template seed content (`ecDefaultContent`, M&F voice) and bundled mockup photography (`public/editorial-calm/`). Browser-verified end-to-end on `mews-demo-hotel` (**renamed "Mason & Fifth" in the dev DB for evaluation — revert after sign-off**). Uncommitted; Karol visual pass pending. Full detail: 2026-06-10 session log.

**Deferred / known gaps:** room tagline subheading (no data field); Phase-2 finalising/poll confirmation states (robust-fulfilment stream); voucher link is visual-only (`?voucher=` carried, no redemption); long-stay panel CTAs are an enquiry mailto (no residency flow); newsletter has no list backend; mockup's multi-house section needs a houses content block (decision pending); `/dev/themes` page stale.

**Design decisions locked during mockup iteration (2026-06-10):**

* Rate & extras page: editorial collage (1 large \+ 2 stacked room photos) at top of left column, then room name \+ tagline subheading \+ description, then dominant rate-plan rows, then extras.
* Extras: rows only, max \~3 visible register below rates — **dotted-leader menu typography** (name … £x / unit, description tucked beneath, small controls).
* Basket: right-rail card (photo, room name, lines, total, CTA) **plus floating corner chip** bottom-right (live total, expands to mini basket card) that persists on every page until checkout.
* Confirm page container matches rate page: 1280px / 360px rail / 60 gap.
* Step dots: 19px circles, 12px mono labels.

**Build plan:**

1. Scaffold `src/themes/editorial-calm/` modelled on Street: `tokens.ts` (paper \#FAF9F5 / ink / hairlines), `fonts.ts` (Hanken Grotesk \+ Newsreader \+ Courier Prime via next/font), `schema.ts` (hero, gallery min 3, per-room galleries — R2), `stripe-appearance.ts`, `EditorialCalmShell.tsx`, `index.ts`.
2. Port screens from mockups, real data via the same hooks Street uses: `Home` ← main.html, `RoomSelect` ← rooms.html, `Extras` ← rate-extras-rows.html, `Checkout` ← confirm.html, `Confirmation` ← Phase2 finalising.
3. Register in `active-theme.ts`, `template-schema.ts`, admin Design tab `TEMPLATES`.
4. Wire `src/app/[property]/*` pages with the editorial-calm branch alongside portico/street.
5. Verify: typecheck \+ build \+ full flow on dev property via `?_template=editorial-calm`; Karol does the visual pass.
