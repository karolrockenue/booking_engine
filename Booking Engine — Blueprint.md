...
> **Companion doc — Ryft payment rail (replacing Stripe).** The booking engine is migrating its payment rail from **Stripe Connect** to **Ryft** (Stripe won't onboard the UAE platform to pay UK hotels; Ryft will). The work stream lives in **`Ryft Migration — Handoff.md`** (repo root) on the unmerged **`ryft-migration`** branch. **Status (2026-06-28): both NR (pay-now) and Flex (refundable, Credential-on-File / off-session MIT) are proven end-to-end live; refunds/cancels are rail-aware.** Migration is additive — Stripe columns/code stay alongside Ryft until a final cutover pass (delete ~65 Stripe files + `stripe_*` columns). Remaining tail: prod webhook + `RYFT_WEBHOOK_SECRET`, the cutover itself, drizzle migrations for the ad-hoc Ryft ALTERs, USD→GBP display fix. If a task touches payments, read that handoff — the references to "Stripe" elsewhere in this blueprint describe `main`, which is pre-migration. Per-property `paymentRail` (`ryftAccountStatus==='active' ? 'ryft' : 'stripe'`) decides which rail a hotel uses.
