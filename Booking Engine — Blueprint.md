...
### Remaining
- **Phase 4 — Mews writes:** customer → reservation (`TimeUnitPrices` + `CheckRateApplicability:false`) → external payment → `reservations/cancel`. **Gated on CQ-1 + CQ-2** (raise with Mews before building writes).
