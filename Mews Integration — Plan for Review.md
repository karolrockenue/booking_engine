...
Use **`getAvailability/2024-01-22`**, which breaks out `UsableResources`, `Occupied`, and out-of-order/`ResourceBlock` components so we **compute the sellable count** ourselves — the right primitive for a write-side engine that must not oversell. Do **not** subtract `Adjustments` from the legacy endpoint: there, `Availabilities` is already the absolute, netted sellable count and `Adjustments` are informational deltas. (Market Pulse uses the legacy endpoint + a separate `resourceBlocks/getAll`; the versioned endpoint is cleaner here.)
...
- **P2 — DB migration + admin Mews connect** (manual token; Service, external payment type, tax mode).
- **P3 — Mews reads** → native tables; reconcile rooms/rates/availability against the Mews UI 1:1 on a M&F demo enterprise.
- **Gate — resolve CQ-1 + CQ-2 with Mews connectivity before writes.**
- **P4 — Mews writes** (customer → reservation with `TimeUnitPrices`/`CheckRateApplicability:false` → products → `payments/addExternal` → `reservations/cancel`); end-to-end test booking on demo with a Stripe test card; capture the sold-out error shape.
